...
printf  "\n${Yellow} Api Install\n ${Color_Off}"

 {
        ...
        printf  "${Green}✓${Color_Off} Api installed\n"
    }  || {
        printf "${Red}x${Color_Off} Nested Caught (@ $__EXCEPTION_LINE__)\n"
        exit 1
    }
while [[ $(kubectl get pods -l app=api -n $Namespace -o 'jsonpath={..status.conditions[?(@.type=="Ready")].status}') != "True" ]]; do printf "." && sleep 1; done
